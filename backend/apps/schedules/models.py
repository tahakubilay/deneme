# backend/apps/schedules/models.py
...
class Vardiya(models.Model):
    ...
    baslangic_zamani = models.DateTimeField(verbose_name="Planlanan Başlangıç Zamanı")
    bitis_zamani = models.DateTimeField(verbose_name="Planlanan Bitiş Zamanı")
    gercek_baslangic_zamani = models.DateTimeField(null=True, blank=True, verbose_name="Gerçek Başlangıç Zamanı")
    gercek_bitis_zamani = models.DateTimeField(null=True, blank=True, verbose_name="Gerçek Bitiş Zamanı")
...
class VardiyaIptalIstegi(models.Model):
    istek_yapan = models.ForeignKey('users.CustomUser', on_delete=models.CASCADE, related_name='gonderdigi_iptal_istekleri')
    vardiya = models.OneToOneField(Vardiya, on_delete=models.CASCADE, related_name='iptal_istegi')
    durum = models.CharField(max_length=30, choices=IstekDurum.choices, default=IstekDurum.ADMIN_ONAYI_BEKLIYOR)
    orijinal_vardiya_durumu = models.CharField(max_length=20, choices=VardiyaDurum.choices, default=VardiyaDurum.TASLAK) # YENİ ALAN
    olusturulma_tarihi = models.DateTimeField(auto_now_add=True)
    guncellenme_tarihi = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"{self.istek_yapan.username} - {self.vardiya} için iptal isteği. Durum: {self.get_durum_display()}"